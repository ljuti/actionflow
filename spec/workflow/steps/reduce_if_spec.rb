...
RSpec.describe Workflow::Steps::ReduceIf do
  it "runs steps when condition is truthy" do
    ran = false
    step = described_class.new(->(ctx) { true }, [->(ctx) { ran = true; ctx }])
  ...
  it "skips steps when condition is falsy" do
    ran = false
    step = described_class.new(->(ctx) { false }, [->(ctx) { ran = true; ctx }])
    step.call(Workflow::Context.new)
  ...
  it "resets skip_remaining on exit" do
    step = described_class.new(->(ctx) { true }, [->(ctx) { ctx.skip_remaining!; ctx }])
    ctx = Workflow::Context.new
    step.call(ctx)
  ...
  it "does not reset skip_remaining on failure" do
    step = described_class.new(->(ctx) { true }, [->(ctx) { ctx.fail!; ctx }])
    ctx = Workflow::Context.new
    step.call(ctx)
  ...
  it "does not reset skip_remaining when skip_all_remaining is set" do
    step = described_class.new(->(ctx) { true }, [->(ctx) { ctx.skip_all_remaining!; ctx }])
    ctx = Workflow::Context.new
    step.call(ctx)
